...
public class Controller implements Initializable {
    ...
    public void btnLogin(ActionEvent e){
        ...
        if(od.login(userID, userPW)==1){
            System.out.println("로그인하였습니다. 로그인된 Main page로 이동");
        } else if(od.login(userID, userPW)==0) {
            System.out.println("로그인에 실패하였습니다. 로그인 화면으로 이동");
        } else if(od.login(userID, userPW)==-1){
    ...
    public void btnChange(ActionEvent e){
        ...
        String newPw = "123456"; // 새로운 비번 입력용 새로운 창이 떠야한다.

        if(od.loginChangeAccount(userID, userName, userPW, newPw)==1){
            System.out.println("비밀번호를 정상적으로 변경하였습니다. 메인 화면으로 이동");
        } else if(od.loginChangeAccount(userID, userName, userPW, newPw)==0) {
            System.out.println("일치하는 정보가 찾을 수 없습니다. 로그인 화면으로 이동");
        } else if(od.loginChangeAccount(userID, userName, userPW, newPw)==-1){
            System.out.println("오류가 발생하였습니다. 로그인 화면으로 이동");
        }
    }
    ...
    public void btnCreate(ActionEvent e){
        ...
        if(od.loginCreateAccount(userID, userName, userPW)==1){
            System.out.println("가입에 성공하였습니다. 메인 화면으로 이동");
        } else if(od.loginCreateAccount(userID, userName, userPW)==0) {
            System.out.println("이미 가입된 아이디입니다. 로그인 화면으로 이동");
        } else if(od.loginCreateAccount(userID, userName, userPW)==-1){
            System.out.println("오류가 발생하였습니다. 로그인 화면으로 이동");
        }
    }
    ...
    public void btnClose(ActionEvent e){
        ...
        if(od.loginCloseAccount(userID, userName, userPW)==1){
            System.out.println("계정을 정상적으로 삭제하였습니다. 메인 화면으로 이동");
        } else if(od.loginCloseAccount(userID, userName, userPW)==0) {
            System.out.println("일치하는 정보를 찾을수 없습니다. 로그인 화면으로 이동");
        } else if(od.loginCloseAccount(userID, userName, userPW)==-1){
            System.out.println("오류가 발생하였습니다. 로그인 화면으로 이동");
        }
    }
